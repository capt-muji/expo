...
Render `<ReleaseTagger />` inside `<PostHogProvider>`. Every subsequent `capture()` carries these properties, so you can filter or group events by `eas/update_id` in PostHog. `eas/account` is set only when your [app config](/workflow/configuration/) sets `owner`; without it, that property has no value.
...
| `eas/channel`         | `expo-updates`   | build or update job   | no link                    |

...
function useActivityFromMetaChildren(meta: MetaNode[]) {
  ...
  const [previousMeta, setPreviousMeta] = React.useState(sortedMeta);
  const [activity, setActivity] = React.useState(() => buildUserActivity(sortedMeta));

  if (!deepObjectCompare(previousMeta, sortedMeta)) {
    setPreviousMeta(sortedMeta);
    setActivity(buildUserActivity(sortedMeta));
  }
